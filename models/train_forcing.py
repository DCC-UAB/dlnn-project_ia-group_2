    ...
    with torch.no_grad():
        for batch_idx, (image, captions) in enumerate(iter(loader)):
            ...
            if model.training and teacher_forcing_prob > 0.0:
                # Use ground truth captions for teacher forcing during validation
                outputs = model(image, captions, teacher_forcing_prob=1.0)
            else:
                # Disable teacher forcing
                outputs = model(image, captions, teacher_forcing_prob=0.0)
